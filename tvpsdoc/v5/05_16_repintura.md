...
O importante é que você entenda como as ferramentas que usa funcionam e se o comportamento delas é compatível com seus objetivos, independentemente de fazerem repintura ou não. Como aprenderá ao ler esta página, _repainting_ é um assunto complexo. Ele tem muitas faces e muitas causas. Mesmo se você não programar em Pine Script™, esta página o ajudará a entender a gama de causas que podem levar ao repintura e, esperamos, possibilitará discussões mais significativas com os autores de scripts.

### Para programadores de Pine Script™

Conforme discutido acima, nem todas as formas de comportamento de _repainting_ devem ser evitadas a todo custo, nem todo comportamento potencial de repintura é necessariamente evitável. Esperamos que esta página o ajude a entender melhor as dinâmicas em jogo para que você possa projetar suas ferramentas de negociação com esses comportamentos em mente. O conteúdo desta página deve ajudar a conscientizá-lo sobre erros de codificação comuns que produzem resultados de repintura enganosos.

Quaisquer que sejam suas decisões de design, se você [publicar](./04_06_publicacao.md) seu script, explique o script aos traders para que eles possam entender como ele se comporta.

Esta página cobre três amplas categorias de causas de _repainting_:

- [Cálculos históricos vs em tempo real](./04_10_repainting.md#cálculos-históricos-vs-em-tempo-real)
- [Plotagem no passado](./04_10_repainting.md#plotagem-no-passado)
- [Variações no conjunto de dados](./04_10_repainting.md#variações-no-conjunto-de-dados)
...
Em barras de tempo real (barras em execução quando o mercado do instrumento está aberto), no entanto, os valores de [máxima](https://br.tradingview.com/pine-script-reference/v5/#var_high), [mínima](https://br.tradingview.com/pine-script-reference/v5/#var_low) e [fechamento](https://br.tradingview.com/pine-script-reference/v5/#var_close) não são fixos; eles podem mudar muitas vezes antes que a barra em tempo real se feche e seus valores HLC

 sejam fixados. Eles são _fluidos_. Isso leva a um script às vezes funcionando de maneira diferente em dados históricos e em tempo real, onde apenas o preço de [abertura](https://br.tradingview.com/pine-script-reference/v5/#var_open) não mudará durante a barra.

Qualquer script que use valores como [máxima](https://br.tradingview.com/pine-script-reference/v5/#var_high), [mínima](https://br.tradingview.com/pine-script-reference/v5/#var_low) e [fechamento](https://br.tradingview.com/pine-script-reference/v5/#var_close) em tempo real está sujeito a produzir cálculos que podem não ser repetíveis em barras históricas — portanto, faz repintura.
...
![Repainting](./imgs/Repainting-01.B4bnEQz8_PQ677.webp)
...
- Estamos testemunhando um script de repintura.
...
Isso pode ser alcançado de várias maneiras. Este método adiciona uma condição `and barstate.isconfirmed` às nossas detecções de cruzamento, que exige que o script esteja executando na última iteração da barra, quando ela se fecha e os preços são confirmados. É uma maneira simples de evitar o repintura:
